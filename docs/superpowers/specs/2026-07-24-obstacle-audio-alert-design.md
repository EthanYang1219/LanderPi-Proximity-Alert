...
- **`aplay` processes are not reaped.** `Popen` is fire-and-forget with no `wait()`/`poll()` sweep, so each finished `aplay` lingers as a `<defunct>` zombie until the node exits. Negligible for a prototype demo (a handful of obstacles); a long unattended run would slowly leak PIDs. Left unhandled intentionally — reaping would add child-tracking state this prototype doesn't warrant.
- **Cross-process `encounter_id` reuse after a partial restart.** `encounter_id` resets (0 → 1) when `path_tracker`'s process restarts, but this node's `_last_played_encounter_id` persists if `obstacle_audio` is left running across that restart. If the new run's first `encounter_id` happens to equal the last one this node played, that single first post-restart alert is suppressed. Only the first post-restart encounter is affected, and only on an id collision; in normal use all terminals restart together. Not handled — acceptable for a prototype.
