...
None beyond what `subprocess.Popen` provides for free. If `aplay` fails (missing file, device busy, bad WAV), it fails in a background process with no retry or alerting — this is an observational prototype, not a safety system, so YAGNI applies.
...
- A thin ROS wrapper (`obstacle_audio.py`) that subscribes to `/avoidance_decision`, calls `should_play`, and spawns `aplay` — tested in-container (needs rclpy), following the existing pattern in `test_decision_logger_csv.py`. The subprocess call itself is not exercised by the test (no real audio hardware assertion in CI-style testing); the test verifies only that `Popen` is invoked with the right arguments when `should_play` returns `True`, and not invoked when it returns `False`.

## Open questions / edge cases considered

- **Overlapping `aplay` processes.** If `should_play` somehow returned `True` twice in rapid succession (shouldn't happen given the `encounter_id` gate, but not architecturally prevented), two `aplay` processes could run concurrently. Not guarded against in this prototype — acceptable given the trigger logic already makes it very unlikely, and the consequence (a doubled-up sound) is cosmetic, not harmful.
- **WAV file missing at runtime.** `aplay` will simply error to its own stderr; the node keeps running and keeps listening for the next encounter. No node-level crash.
