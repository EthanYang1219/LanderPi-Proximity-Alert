...
class AvoidanceConfig: # All measurements are in meters or m/s for the speed. All timeouts/time durations are in seconds
    ...
    side_window_deg: float = 70.0          # Angular width of the LEFT/RIGHT clearance windows (centered at +/-90 deg). Wider = clearance check considers more of the flank
    rear_window_deg: float = 70.0         # Angular width of the REAR clearance window (centered behind). Wider = more of the back considered when reversing/recovering
    ...
    rear_clearance_min: float = 0.20       # Minimum REAR clearance required to allow the reverse nudge during TURN/RECOVER. Lower = reverses even with less room behind
    ...
    heading_kp: float = 5.0                # Heading-hold PID proportional gain. Higher = snappier correction toward goal/target heading, more prone to overshoot/oscillation
    heading_ki: float = 0.001                # Heading-hold PID integral gain. Nonzero corrects small steady-state heading bias, but risks windup/overshoot if too high
    ...
    centering_timeout: float = 7.5         # Max seconds to spend centering before declaring arrival regardless of remaining offset. Guarantees the run terminates when the correction is gated off or the flank is blocked; the leftover offset is then measured on the floor instead
